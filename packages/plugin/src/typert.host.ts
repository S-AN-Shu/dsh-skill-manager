...
import { RPC_SCHEMA_VERSION } from "./rpc.js";

const PACKAGE_NAME = "dsh-skill-manager";
const SERVICE_NAME = "skillManager";

const skillTargetSchema = z.enum(["dsh", "codex", "claude", "agents", "opencode"]);
const skillOriginSchema = z.enum([
  "self",
  "local-import",
  "github",
  "skills-sh",
...
const localImportSourceSchema = z.object({
...
const versionedRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION)
}).strict();
...
const repositorySortSchema = z.enum(["popular", "latest", "trend-weekly", "trend-monthly", "relevance"]);
const repositoryIdentitySchema = z.object({
  owner: z.string().regex(/^[A-Za-z0-9_.-]+$/u),
  name: z.string().regex(/^[A-Za-z0-9_.-]+$/u)
}).strict();
const getCapabilitiesRequestSchema = versionedRequestSchema;
const searchRepositoriesRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
...
const browseRepositoriesRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  sort: repositorySortSchema.optional(),
  page: z.number().int().min(1).max(10).optional(),
  limit: z.number().int().min(1).max(100).optional()
}).strict();
const inspectRepositoryRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
...
const skillIntentSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  repository: repositoryIdentitySchema,
  skillPath: z.string().min(1).max(512).refine((path) => path === "." || (
    !path.startsWith("/") && !path.includes("\\")
    && path.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== "..")
  ), "Unsafe Skill path"),
  acknowledgeHighRisk: z.boolean().optional()
}).strict();
const safeSkillPathSchema = z.string().min(1).max(512).refine((path) => path === "." || (
  !path.startsWith("/") && !path.includes("\\")
  && path.split("/").every((segment) => segment.length > 0 && segment !== "." && segment !== "..")
), "Unsafe Skill path");
const installRepositoryRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  repository: repositoryIdentitySchema,
  selection: z.union([
...
const skillClassificationSchema = z.object({
...
const repositoryTrendSchema = z.object({
...
const repositoryQueryResultSchema = z.object({
...
const marketplacePartyV2Schema = z.object({ name: z.string(), url: z.string().nullable() }).strict();
...
const repositoryInspectionSchema = z.object({
  ...
  readme: z.object({
    path: z.string(), title: z.string().nullable(), content: z.string(), blobSha: z.string().regex(/^[a-f0-9]{40}$/iu)
  }).strict().nullable(),
...
const riskAssessmentSchema = z.object({
  ...
  findings: z.array(z.object({
    code: z.string(), severity: z.enum(["info", "warning", "high"]), title: z.string(), detail: z.string(), file: z.string()
  }).strict()),
  scannerVersion: z.string()
}).strict();
const repositoryInspectionResultSchema = z.object({
  inspection: repositoryInspectionSchema,
  assessments: z.array(z.object({
    skillPath: z.string(), assessment: riskAssessmentSchema
  }).strict())
}).strict();
const repositoryInstallResultSchema = z.object({
  skillPath: z.string(),
...
const resolveMediaRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  source: mediaSourceSchema
}).strict();
const capabilitiesSchema = z.object({
  ...
  buildId: z.string(),
  features: z.object({
    marketplaceV2: z.boolean(), repositoryInspection: z.boolean(), mediaProxy: z.boolean(),
    indexCatalog: z.boolean(), riskAssessment: z.boolean(), githubTrending: z.boolean(), skillClassification: z.boolean(),
    provenanceV2: z.boolean(), updateRiskGate: z.boolean(), repositoryBatchAnalysis: z.boolean(),
    repositoryBatchInstall: z.boolean(), batchProvenance: z.boolean(), skillsShDiscoveryHints: z.boolean()
  }).strict()
}).strict();
const searchMarketplaceRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  query: z.string(),
  limit: z.number().int().optional()
}).strict();
const browseMarketplaceRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  offset: z.number().int().min(0).max(100_000).optional(),
  limit: z.number().int().min(1).max(200).optional()
}).strict();
const marketplacePartySchema = z.object({
  name: z.string(),
...
const resolveMarketplaceRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  entry: marketplaceEntrySchema
}).strict();
const installMarketplaceRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  entry: marketplaceEntrySchema
}).strict();
const verifyProvenanceRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  name: z.string()
}).strict();
const checkUpdatesRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
...
const listBackupsRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  name: z.string().optional()
}).strict();
const rollbackSkillRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  name: z.string(),
  backupId: z.uuid()
}).strict();
...
const restoreTrashRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  name: z.string(),
  trashId: z.uuid()
}).strict();
const externalTargetSchema = z.enum(["codex", "claude", "agents", "opencode"]);
const discoverExternalRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  targets: z.array(externalTargetSchema).optional()
}).strict();
const importExternalRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  target: externalTargetSchema,
  name: z.string()
}).strict();
const listTargetStatesRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  names: z.array(z.string()).optional(),
  targets: z.array(externalTargetSchema).optional()
}).strict();
const setTargetEnabledRequestSchema = z.object({
  schemaVersion: z.literal(RPC_SCHEMA_VERSION),
  name: z.string(),
  target: externalTargetSchema,
  enabled: z.boolean()
}).strict();
const resolvedMarketplaceEntrySchema = marketplaceEntrySchema.extend({
  description: z.string(),
  repository: z.object({
    host: z.literal("github"),
...
const marketplaceBrowseResultSchema = z.object({
  source: z.literal("skills-sh"),
...
const trashedSkillSchema = z.object({
  name: z.string(),
  trashId: z.uuid(),
  description: z.string(),
  origin: skillOriginSchema,
...
const provenanceVerificationSchema = z.object({
  name: z.string(),
  status: z.enum(["matched", "custom", "ambiguous", "ineligible"]),
  skill: managedSkillSchema
}).strict();
const provenanceBatchFailureSchema = z.object({
  name: z.string(),
  code: z.string(),
  message: z.string()
}).strict();
...
export const skillManagerDescriptors = [
  descriptor(
    "list",
  ...
  descriptor("getCapabilities", getCapabilitiesRequestSchema, z.union([
    successSchema(z.object({ capabilities: capabilitiesSchema }).strict()), failureSchema
  ])),
  descriptor("searchRepositories", searchRepositoriesRequestSchema, z.union([
    successSchema(z.object({ result: repositoryQueryResultSchema }).strict()), failureSchema
  ])),
  ...
  descriptor("inspectRepository", inspectRepositoryRequestSchema, z.union([
    successSchema(repositoryInspectionResultSchema), failureSchema
  ])),
  descriptor("installSkill", skillIntentSchema, z.union([
    successSchema(z.object({ skill: managedSkillSchema }).strict()), failureSchema
  ])),
  descriptor("installRepository", installRepositoryRequestSchema, z.union([
    successSchema(z.object({ results: z.array(repositoryInstallResultSchema) }).strict()), failureSchema
  ])),
  descriptor("assessSkillRisk", skillIntentSchema, z.union([
    successSchema(z.object({ assessment: riskAssessmentSchema }).strict()), failureSchema
  ])),
  descriptor("resolveMedia", resolveMediaRequestSchema, z.union([
    successSchema(z.object({ asset: mediaAssetSchema }).strict()), failureSchema
  ])),
  descriptor(
    "verifyProvenance",
