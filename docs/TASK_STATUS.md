...
## Active Objective - Public GitHub DSH Plugin Release - 2026-08-25
...
- Verified baseline: the current plugin already exposes a `dsh.bundle.patch`, a stable Cordis patch id, a Typert Host service, and a public `settings.section` Client entry tested against Harness `0.1.1-rc.2` / DSH Desktop v0.5.4. The public repository does not yet exist, the workspace has no Git remote, package versions remain `0.0.0`, the Host config lacks an exported Schemastery `Config`, the Client-owned stylesheet is not disposed on unload, and GitHub-source installation is not self-contained from the monorepo root.
...
- Current status: in progress. Terra completed the official-rule study. Repository, lifecycle, packaging, public-history, and privacy audits are underway; no GitHub repository or release has been created yet.
